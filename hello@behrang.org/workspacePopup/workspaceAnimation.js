...
const MonitorGroup = GObject.registerClass(
	...
	class MonitorGroup extends St.Widget {
		get baseDistance() {
			...
			else return this._monitor.width + spacing;
		}
		...
		getWorkspaceProgress(workspace) {
			const group = this._workspaceGroups.find(
				(g) => g.workspace.index() === workspace.index(),
			);
		...
		_interpolateProgress(progress, monitorGroup) {
			if (this.index === monitorGroup.index) return progress;
			...
			if (points1[upper] === points1[lower]) return points2[upper];
		...
		_init(monitor, workspaceIndices, movingWindow) {
			...
			for (const i of workspaceIndices) {
				...
				const fullscreen = ws
					.list_windows()
					.some((w) => w.get_monitor() === monitor.index && w.is_fullscreen());
				...
				if (targetRow > fromRow) y += this.baseDistanceY;
				else if (targetRow < fromRow) y -= this.baseDistanceY;

				if (targetColumn > fromColumn) x += this.baseDistanceX;
				else if (targetColumn < fromColumn) x -= this.baseDistanceX;
			}
		...
		get progress() {
			...
			if (targetRow > fromRow) return -this._container.y / this.baseDistanceY;
		...
		_getWorkspaceGroupProgress(group) {
			...
			if (targetRow > fromRow) return group.y / this.baseDistanceY;
			else if (targetRow < fromRow) return -group.y / this.baseDistanceY;

			if (targetColumn > fromColumn) return group.x / this.baseDistanceX;
			else if (targetColumn < fromColumn) return -group.x / this.baseDistanceX;
		}
	},
);
...
export class WorkspaceAnimationController extends GWorkspaceAnimationController {
	_prepareWorkspaceSwitch(workspaceIndices) {
		...
		if (!workspaceIndices) workspaceIndices = [...Array(nWorkspaces).keys()];
