	...
	class WorkspaceSwitcherPopupList extends St.BoxLayout {
		_init(thumbnails, workspaceName, options) {
			...
			for (let i = 0; i < this._rows; i++) {
				const workspacesRow = new St.BoxLayout({
				...
				workspacesRow.connect("style-changed", () => {
					...
					workspacesRow.spacing = workspacesRow
						.get_theme_node()
						.get_length("spacing");
					this.redisplay();
				});
			...
			this._activeWorkspaceChangedId = workspaceManager.connect(
		...
		addItem(thumbnail, workspaceName) {
			...
			const list = this._lists[Math.floor(this._items.length / this._columns)];
			const bbox = new SwitcherButton(this._childWidth, this._childHeight);
			...
			if (this._showWorkspaceName) {
				const labelBox = new SwitcherButton(
					this._childWidth,
					this._childHeight,
				);
				labelBox.set_child(
		...
		redisplay() {
			...
			for (let i = 0; i < this._items.length; i++) {
				const bbox = this._items[i];
				bbox.setSize(this._childWidth, this._childHeight);

				const leftPadding = bbox.get_theme_node().get_padding(St.Side.LEFT);
				const rightPadding = bbox.get_theme_node().get_padding(St.Side.RIGHT);
				const topPadding = bbox.get_theme_node().get_padding(St.Side.TOP);
				const bottomPadding = bbox.get_theme_node().get_padding(St.Side.BOTTOM);

				for (
					let itemIndex = 0;
					itemIndex < bbox.get_child().get_children().length;
					itemIndex++
				) {
					const item = bbox.get_child().get_children()[itemIndex];
					if (item instanceof WorkspaceThumbnail) {
						// 2 is magic number. Can not find the reason for it.
						item.setScale(
							(bbox.get_width() - leftPadding - rightPadding - 2) /
								item.get_width(),
							(bbox.get_height() - topPadding - bottomPadding - 2) /
								item.get_height(),
						);
					}
					if (item instanceof SwitcherButton) {
						item.setSize(
							this._childWidth - leftPadding - rightPadding,
							this._childHeight - topPadding - bottomPadding,
						);
		...
		highlight(index, justOutline) {
			...
			if (this._items[index]) {
				this._items[index].add_style_pseudo_class(
					justOutline ? "highlighted" : "selected",
				);
			}
		...
		get_preferred_child_size() {
			...
			const ratio = workArea.width / workArea.height;
		...
		vfunc_get_preferred_height(_forWidth) {
			const padding =
				this.get_theme_node().get_padding(St.Side.TOP) +
				this.get_theme_node().get_padding(St.Side.BOTTOM);

			this._height =
				(this.get_preferred_child_size().height + this.spacing) * this._rows -
				this.spacing;
		...
		vfunc_get_preferred_width(_forHeight) {
			const padding =
				this.get_theme_node().get_padding(St.Side.RIGHT) +
				this.get_theme_node().get_padding(St.Side.LEFT);

			this._width =
				(this.get_preferred_child_size().width + this._lists[0].spacing) *
					this._columns -
				this._lists[0].spacing;
